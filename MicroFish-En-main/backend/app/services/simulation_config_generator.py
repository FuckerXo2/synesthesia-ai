...
Uses an LLM to automatically generate detailed simulation parameters based on the simulation requirement,
document content, and graph info. Achieves end-to-end automation without manual parameter setup.

Uses a step-by-step generation strategy to avoid failures caused by generating too much at once:
...
# Reference daily-schedule configuration for China (Beijing time)
CHINA_TIMEZONE_CONFIG = {
    # Dead-of-night hours (almost no activity)
    ...
    # Late-night hours (activity declining)
    "night_hours": [23],
...
class AgentActivityConfig:
    ...
    # Posting frequency (expected counts per hour)
...
class TimeSimulationConfig:
    ...
    total_simulation_hours: int = 72  # default simulates 72 hours (3 days)

    # Time represented by each round (simulation minutes) - default 60 minutes (1 hour), accelerates time flow
...
class PlatformConfig:
    ...
    # Viral threshold (how many interactions are needed to trigger spread)
    viral_threshold: int = 10

    # Echo chamber strength (degree of clustering around similar viewpoints)
...
class SimulationConfigGenerator:
    ...
    Uses the LLM to analyze the simulation requirement, document content, and graph entity info,
    ...
    # Context truncation lengths (in characters) per step
    ...
    def generate_config(
        ...
        """
        logger.info(f"Starting intelligent generation of simulation config: simulation_id={simulation_id}, entities={len(entities)}")
    ...
    def _build_context(
        ...
        remaining_length = self.MAX_CONTEXT_LENGTH - current_length - 500  # leave a 500-char margin
    ...
    def _summarize_entities(self, entities: List[EntityNode]) -> str:
        ...
        for e in entities:
            tkey = e.get_entity_type() or "Unknown"
            if tkey not in by_type:
                by_type[tkey] = []
            by_type[tkey].append(e)
    ...
    def _generate_time_config(self, context: str, num_entities: int) -> Dict[str, Any]:
        ...
        # Compute the maximum allowed value (90% of the number of agents)
...
## Task
Please produce a time configuration as JSON.

### Basic principles (for reference only; adjust flexibly based on the specific event and participant groups):
- Infer the time zone and daily schedule of the target user group from the simulation scenario; the following is a reference example for UTC+8
- 0-5am: almost no activity (activity multiplier 0.05)
- 6-8am: gradually active (activity multiplier 0.4)
- 9am-6pm: moderate activity during work hours (activity multiplier 0.7)
- 7-10pm: peak activity (activity multiplier 1.5)
- After 11pm: activity declines (activity multiplier 0.5)
- General pattern: low activity in the early morning, gradually rising in the morning, moderate during work hours, peaking in the evening
- **Important**: the example values below are for reference only; adjust the specific time buckets based on the nature of the event and the characteristics of the participant groups
  - For example: a student group's peak may be 9-11pm; media are active all day; official agencies are active only during work hours
  - For example: a breaking hot event may produce late-night discussions, so off_peak_hours may be shortened appropriately
...
- peak_hours (int array): peak hours, adjusted to the event's participant groups
...
- reasoning (string): brief explanation of why this configuration was chosen"""
        ...
        system_prompt = "You are a social media simulation expert. Return pure JSON; the time configuration must match the daily-schedule habits of the target user group in the simulation scenario."
    ...
    def _get_default_time_config(self, num_entities: int) -> Dict[str, Any]:
        ...
        return {
            "total_simulation_hours": 72,
            "minutes_per_round": 60,  # 1 hour per round to accelerate simulation time
    ...
    def _parse_time_config(self, result: Dict[str, Any], num_entities: int) -> TimeSimulationConfig:
        """Parse the time configuration result and validate that agents_per_hour values do not exceed the total agent count"""
        # Get raw values
        agents_per_hour_min = result.get("agents_per_hour_min", max(1, num_entities // 15))
        agents_per_hour_max = result.get("agents_per_hour_max", max(5, num_entities // 5))

        # Validate and correct: do not exceed the total agent count
    ...
    def _generate_event_config(
        ...
        type_info = "\n".join([
            f"- {tname}: {', '.join(examples)}"
            for tname, examples in type_examples.items()
        ])
...
## Task
Please produce the event configuration JSON:
- Extract hot topic keywords
- Describe the narrative direction of public opinion
- Design initial post content; **each post must specify poster_type (the publisher's type)**

**Important**: poster_type must be chosen from the "Available entity types" above so that initial posts can be assigned to a suitable agent.
...
{{
    ...
    def _assign_initial_post_agents(
        ...
        # Type alias table (handle different formats the LLM may output)
        type_aliases = {
        ...
        for post in event_config.initial_posts:
            ...
            if matched_agent_id is None:
                ...
                if agent_configs:
                    # Sort by influence in descending order and pick the top
    ...
    def _generate_agent_configs_batch(
        ...
        # Build entity info (use the configured summary length)
        entity_list = []
...
## Task
Generate an activity configuration for each entity, noting:
- **Time should match the daily schedule of the target user group**: the following is a reference (UTC+8); adjust based on the simulation scenario
...
{{
    "agent_configs": [
        ...
        system_prompt = "You are an expert in social media behavior analysis. Return pure JSON; the configuration must match the daily-schedule habits of the target user group in the simulation scenario."
        ...
        for i, entity in enumerate(entities):
            ...
            # If the LLM did not produce one, generate via rules
            if not cfg:
    ...
    def _generate_agent_config_by_rule(self, entity: EntityNode) -> Dict[str, Any]:
        """Generate a single agent's configuration by rules (Chinese daily schedule)"""

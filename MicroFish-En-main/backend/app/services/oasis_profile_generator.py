...
2. Optimized prompts to produce very detailed personas
...
class OasisProfileGenerator:
    ...
    Converts entities from the Zep graph into the Agent Profiles required by the OASIS simulation

    Optimizations:
    1. Call Zep graph retrieval to obtain richer context
    2. Generate very detailed personas (including basic info, career experience, personality, social media behavior, etc.)
    3. Distinguish between individual entities and abstract group entities
    """
    ...
    # Group/organization entity types (need representative account personas)
    GROUP_ENTITY_TYPES = [
    ...
    def generate_profile_from_entity(
        ...
        Args:
            ...
            use_llm: Whether to use the LLM to produce a detailed persona
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        Zep does not have a built-in hybrid search endpoint, so we search edges and nodes separately and merge the results.
        Runs the two requests in parallel for efficiency.
        ...
        # graph_id is required to perform the search
        ...
        try:
            ...
            logger.info(f"Zep hybrid retrieval complete: {entity_name}, obtained {len(results['facts'])} facts and {len(results['node_summaries'])} related nodes")
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        """
        Build the full context for the entity

        Includes:
        1. The entity's own edge info (facts)
        2. Detailed info for related nodes
        3. Rich information obtained via Zep hybrid retrieval
        """
        context_parts = []
        ...
        # 3. Detailed info for related nodes
        ...
        if zep_results.get("facts"):
            # Dedupe: exclude facts already present
    ...
    def _is_individual_entity(self, entity_type: str) -> bool:
        """Check whether an entity type represents an individual"""
        return entity_type.lower() in self.INDIVIDUAL_ENTITY_TYPES

    def _is_group_entity(self, entity_type: str) -> bool:
        """Check whether an entity type represents a group/organization"""
    ...
    def _generate_profile_with_llm(
        ...
        - Individual entity: generate a concrete character persona
        - Group/organization entity: generate a representative account persona
        """
        ...
        # Try multiple times until successful or maximum retries reached
        max_attempts = 3
        last_error = None
        ...
        logger.warning(f"Failed to generate persona via LLM ({max_attempts} attempts): {last_error}, falling back to rule-based generation")
        return self._generate_profile_rule_based(
            entity_name, entity_type, entity_summary, entity_attributes
        )

    def _fix_truncated_json(self, content: str) -> str:
        """Repair truncated JSON (when the output was cut off by max_tokens)"""
        import re
        ...
        # Simple heuristic: if the last char is not a comma or a closing bracket, the string may be truncated
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        # 1. First, fix possible truncation
        content = self._fix_truncated_json(content)
        ...
        if json_match:
            ...
            # Find all string values and replace the newlines inside them
        ...
        # 7. Complete failure: return a baseline structure
        logger.warning(f"JSON repair failed, returning baseline structure")
        return {
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        ...
        base_prompt = "You are an expert in generating social media user personas. Produce detailed, realistic personas for public opinion simulation, reproducing the existing real-world situation as faithfully as possible. You must return valid JSON; no string value may contain unescaped newlines."
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, reproducing the existing real-world situation as faithfully as possible.
...
Please produce JSON containing the following fields:

1. bio: social media bio, around 200 characters
2. persona: detailed persona description (around 2000 characters of plain text) that includes:
   ...
   - Stances and opinions (attitudes toward topics, content likely to provoke/move them)
   - Unique traits (catchphrases, special experiences, personal hobbies)
   - Personal memory (an important part of the persona: describe the individual's connection to the event, as well as their existing actions and reactions within the event)
...
- {get_language_instruction()} (the gender field must be in English: male/female)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account persona for an organization/group entity, reproducing the existing real-world situation as faithfully as possible.
...
2. persona: detailed account description (around 2000 characters of plain text) that includes:
   - Organization basic info (official name, nature of the organization, founding background, main functions)
   - Account positioning (account type, target audience, core functions)
   - Speaking style (language traits, commonly used expressions, taboo topics)
   - Published content characteristics (content types, posting frequency, active time windows)
   - Stances and attitudes (official positions on core topics, how controversy is handled)
   - Additional notes (profile of the group it represents, operational habits)
   - Organization memory (an important part of the persona: describe the organization's connection to the event, and its existing actions and reactions within the event)
3. age: fixed at 30 (virtual age of an organization account)
...
- persona must be a single coherent paragraph; do not use newline characters
...
- The account's voice must match its identity and positioning"""
    ...
    def _generate_profile_rule_based(
        ...
        # Produce different personas based on entity type
        ...
        if entity_type_lower in ["student", "alumni"]:
            ...
            return {
                ...
                "age": 30,  # virtual age of an organization
                "gender": "other",  # organizations use "other"
                "mbti": "ISTJ",  # organizational style: rigorous/conservative
                "country": "China",
                "profession": "Media",
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            graph_id: Graph ID; used by Zep retrieval for richer context
            parallel_count: Number of profiles to generate in parallel, default 5
            realtime_output_path: File path for realtime writes (if provided, writes after each generation)
        ...
        completed_count = [0]  # use a list so it can be mutated from the closure
        lock = Lock()

        # Helper function for realtime writing
        def save_profiles_realtime():
            """Save the already-generated profiles to a file in realtime"""
            ...
            with lock:
                ...
                try:
                    ...
                    logger.warning(f"Realtime save of profiles failed: {e}")
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    # Realtime write to file (even for fallback personas)
                    save_profiles_realtime()
    ...
    def _print_generated_profile(self, entity_name: str, entity_type: str, profile: OasisAgentProfile):
        """Print a generated persona to the console in realtime (full content, not truncated)"""
        separator = "-" * 70

        # Build the complete output content (no truncation)
        ...
        # Print only to the console (avoid duplication; logger no longer prints full content)
        print(output)
    ...
    def save_profiles(
        ...
        """
        Save profiles to a file (choose the correct format based on platform)
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter profiles in CSV format (matches the official OASIS requirement)

        CSV fields required by OASIS Twitter:
        - user_id: user ID (0-based according to CSV order)
    ...
    def _normalize_gender(self, gender: Optional[str]) -> str:
        ...
        # Mapping (including Chinese inputs)
        gender_map = {
            "男": "male",
            "女": "female",
            "机构": "other",
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        Uses the same format as to_reddit_format() to ensure OASIS reads it correctly.
        ...
        for idx, profile in enumerate(profiles):
            ...
            item = {
                ...
                # OASIS-required fields - ensure every one has a default value

...
def download_report(report_id: str):
    ...
    try:
        ...
        if not os.path.exists(md_path):
            # If the MD file does not exist, create a temporary file
            import tempfile
...
def chat_with_report_agent():
    ...
    The Report Agent can autonomously invoke retrieval tools during the chat to answer questions.
...
def get_report_sections(report_id: str):
    ...
    The frontend can poll this endpoint to fetch sections that have already been
    generated, without waiting for the entire report to complete.
...
def check_report_status(simulation_id: str):
    ...
    try:
        ...
        # Only unlock interview after the report completes
...
def get_agent_log(report_id: str):
    ...
    Real-time access to each step of the report generation process, including:
...
def stream_agent_log(report_id: str):
    """
    Get the full Agent log (fetch everything at once)
...
def get_console_log(report_id: str):
    ...
    Real-time access to the console output (INFO, WARNING, etc.) during report
    generation. This differs from the agent-log endpoint (which returns structured
    JSON logs); this is plain-text console-style logs.
...
def stream_console_log(report_id: str):
    """
    Get the full console log (fetch everything at once)

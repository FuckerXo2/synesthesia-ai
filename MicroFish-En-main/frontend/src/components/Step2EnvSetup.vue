...
// State
const phase = ref(0) // 0: initializing, 1: generating personas, 2: generating config, 3: completed
...
const useCustomRounds = ref(false) // Default uses auto-generated round count
...
const autoGeneratedRounds = computed(() => {
  if (!simulationConfig.value?.time_config) {
    return null // Return null if the config hasn't been generated yet
  }
  ...
  // Ensure max rounds is no less than 40 (recommended) to avoid slider range glitches
...
// Look up the corresponding username for an agent_id
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    // User-customized rounds; pass the max_rounds param
    params.maxRounds = customMaxRounds.value
    addLog(t('log.startSimCustomRounds', { rounds: customMaxRounds.value }))
  } else {
    // User opted to keep auto-generated rounds; do not pass max_rounds
...
// Automatically begin preparing the simulation
const startPrepareSimulation = async () => {
  ...
  // Mark step 1 complete and begin step 2
  phase.value = 1
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Immediately set the expected total number of agents (from the prepare API response)
      ...
      startPolling()
      // Begin real-time fetching of Profiles
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Only update when the API returns a valid value; avoid overwriting an existing valid value
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        // If generation is fully complete
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // Config not yet generated; begin polling
...
onMounted(() => {
  // Automatically begin the preparation flow
  if (props.simulationId) {
